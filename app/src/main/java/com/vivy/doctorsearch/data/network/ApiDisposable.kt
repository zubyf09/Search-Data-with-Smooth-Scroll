    ...
    override fun onError(e: Throwable) {
        ...
        when (e) {
            is HttpException -> {
                ...
                failure(ApiError(ApiStatus.BAD_RESPONSE, e.code(), errorMessage))
            }
            is SocketTimeoutException -> {
                failure(ApiError(ApiStatus.TIMEOUT, message = e.localizedMessage))
            }
            is IOException -> {
                failure(ApiError(ApiStatus.NO_CONNECTION, message = e.localizedMessage))
            }
            is NullPointerException -> {
                failure(ApiError(ApiStatus.EMPTY_RESPONSE))
            }

            else -> failure(ApiError(ApiStatus.NOT_DEFINED))
        }
    }
